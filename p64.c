...
#define BufferContents() (mwtell() + BufferOffset - (((CurrentGOB * NumberMDU) + CurrentMDU) * Rate * FrameSkip / (NumberGOB * NumberMDU * FrameRate / FrameRateDiv)))
...
char** argv;
{
    ...
    if (!CImage->StreamFileName) {
        if (!(CImage->StreamFileName = (char*)calloc(strlen(CFrame->ComponentFilePrefix[0]) + 6,
                  sizeof(char)))) {
            WHEREAMI();
...
static void CallOracle(value) int value;
{
    BEGIN("CallOracle");
...
void p64EncodeFrame()
{
    ...
    if (Rate) {
        if (CurrentFrame == StartFrame) { /* Begin Buffer at 0.5 size */
...
void p64EncodeGOB()
{
    ...
    LastMBA = -1;
    MType = 0;
    for (; CurrentMDU < NumberMDU; CurrentMDU++) { /* MAIN LOOP */
        LastMType = MType;
        if ((Rate) && (CurrentMDU) && !(CurrentMDU % QUpdateFrequency) && (CurrentFrame != StartFrame)) { /* Begin Buffer control */
            ExecuteQuantization();
        ...
        {
            if ((VAR < 64) || (VAROR > VAR)) { /* (MC+Inter)mode */
                if ((xValue < 1.0) || ((xValue < 3.0) && (yValue > (xValue * 0.5))) || ((yValue > (xValue / 1.1))))
                    MType = 2; /* Inter mode */
    ...
    if (CFrame) {
        ...
        for (i = 0; i < CFrame->NumberComponents; i++) {
            printf("Component: FilePrefix: %s FileSuffix: %s\n",
                ((*CFrame->ComponentFilePrefix[i]) ? CFrame->ComponentFilePrefix[i] : "Null"),
                ((*CFrame->ComponentFileSuffix[i]) ? CFrame->ComponentFileSuffix[i] : "Null"));
...
void PrintFrameStatistics()
{
    ...
    printf("MV StepSize: %f  MV NumberNonZero: %f  MV NumberZero: %f\n",
        ...
        (double)((double)NumberNZ) / ((double)(NumberGOB * NumberMDU * 6)),
        (double)((double)(NumberGOB * NumberMDU * 6 * 64) - NumberNZ) / ((double)(NumberGOB * NumberMDU * 6)));
    printf("Code MType: ");
    for (x = 0; x < 10; x++)
        printf("%5d", x);
    printf("\n");
    printf("Macro Freq: ");
    for (x = 0; x < 10; x++)
        printf("%5d", MacroTypeFrequency[x]);
    printf("\n");
    printf("Y     Freq: ");
    for (x = 0; x < 10; x++)
        printf("%5d", YTypeFrequency[x]);
    printf("\n");
    printf("UV    Freq: ");
    for (x = 0; x < 10; x++)
        printf("%5d", UVTypeFrequency[x]);
    printf("\n");
    Statistics();
}
